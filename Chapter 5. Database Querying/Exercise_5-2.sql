...
SELECT `users`.`id`, `users`.`name`, `trains`.`source`, Count(*) AS `train_count`, Sum(`trains`.`distance`)/10 AS `total_distance`
FROM `tickets`
...
SELECT `types`.`name` AS `type`, `src`.`name` AS `src_stn`, `dst`.`name` AS `dst_stn`, `trains`.`arrival`, `trains`.`destination`, Round(`types`.`fare_rate`*`trains`.`distance`/1000 - 2) AS `fare`
FROM `trains`
INNER JOIN `types` ON `types`.`id` = `trains`.`type`
INNER JOIN `stations` AS `src` ON `src`.`id` = `trains`.`source`
INNER JOIN `stations` AS `dst` ON `dst`.`id` = `trains`.`destination`
